...
/**
 * 플레이어가 공유할 수 있는 탐사 위치(X, Depth)를 표시합니다.
 */
export const EquipmentInfo: React.FC<EquipmentInfoProps> = React.memo(({ pos, depth }) => {
  return (
    <div className="pixel-badge pixel-font hidden md:flex items-center gap-2 px-3 py-2 z-10 opacity-80 hover:opacity-100 transition-opacity">
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[10px] font-bold text-[#a89065]">Survey</span>
      </div>
      ...
      <div className="text-xs lg:text-sm font-black text-[#f4dfb8]">
        ...
        <span className="text-[#d0b886]/55">D</span>{' '}
        <span className="text-amber-400">{Math.floor(depth)}m</span>
      </div>
    </div>
  );
});
