...
interface TabEffectsProps {
  stats: PlayerStats;
  ownedEffects: EffectDefinition[];
  selectedKey: string | null;
  onSelectKey: (key: string | null) => void;
  selectedEffect: EffectDefinition | null;
}

const TabEffects = ({ stats, ownedEffects, selectedKey, onSelectKey, selectedEffect }: TabEffectsProps) => {
  return (
    <>
              ...
              {selectedEffect.bonus && (
                <div className="text-[16px] text-orange-500 font-black tracking-[0.2em] mb-2">
                  {(() => {
                    ...
                    const stacks = stats.collectionHistory?.[selectedEffect.id] || 0;
                    const totalValue = selectedEffect.bonus.value * stacks;
                    const percentStats = ['critRate', 'critDamage', 'miningSpeed', 'masteryExp'];
                    const isPercent = percentStats.includes(selectedEffect.bonus.stat);
                    const formattedValue = isPercent ? `${(totalValue * 100).toFixed(1)}%` : totalValue.toLocaleString();
                    
                    return `${statMap[selectedEffect.bonus.stat] || selectedEffect.bonus.stat} +${formattedValue}`;
                  })()}
                </div>
              )}

              <div className="text-lg md:text-xl text-white font-black leading-tight">
                {selectedEffect.effectDescription}
              </div>
            </div>
          </div>
        ) : (
